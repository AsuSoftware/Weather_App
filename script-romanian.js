...
class WeatherInfo {
    ...
    createError() {
        ...
        const textError = document.createTextNode("Hopa! Pagina nu a fost gasita");
        ...
        const textDescription = document.createTextNode(
            "Se pare că pagina nu a fost găsită, deoarece este posibil să fi tastat locația greșită sau serverul nu răspunde în acest moment"
        );
    ...
    conditionForTime() {
        ...
        if (this.hours > 0 && this.hours <= 12 || this.hours == '00') {
            //AM
            this.format = "AM";
            const condition = this.conditionForMinutes("Buna dimineata");
            return condition;
        } else if (this.hours > 12 && this.hours <= 19) {
            this.format = "PM";
            const condition = this.conditionForMinutes("Buna ziua");
            return condition;
        } else if (this.hours > 19) {
            this.format = "PM";
            const condition = this.conditionForMinutes("Bună seara");
            return condition;
        }
    }
    ...
    conditionForMinutes(text) {
        if (this.date.getMinutes() < 10) {
            ...
            const descriptionText = document.createTextNode(
                `${text} ${localStorage.name}, vremea de astăzi spune că este ${
            this.data.weather[0].description
          }. Ora exactă pentru această locație este : ${this.date.getHours() - 1}:0${this.date.getMinutes()} ${
            this.format
          }`
            );
            ...
            const descriptionText = document.createTextNode(
                `${text} ${localStorage.name}, vremea de astăzi spune că este ${
            this.data.weather[0].description
          }. Ora exactă pentru această locație este : ${this.date.getHours() - 1}:${this.date.getMinutes()} ${
            this.format
          }`
            );
            return descriptionText;
    ...
    createFormBody() {
        ...
        const titleText = document.createTextNode("Bine ai venit pe aplicația meteo");
        title.appendChild(titleText);
        form.appendChild(title);
        const titleName = this.createElements("h2", "title-name");
        const textTitleName = document.createTextNode("Cum te numești?");
        ...
        const textTitleLocation = document.createTextNode("Care este locația dvs?");
