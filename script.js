...
class WeatherInfo {
  constructor() {
    // 2
    this.localName = window.localStorage.getItem("name");         //Read Storage
    this.localLocation = window.localStorage.getItem("location"); //Read Storage
    this.checkLocal();                                            //Check if in local have something
  }

  //3
  checkLocal() {
    if (this.localName == null && this.localLocation == null) { //if..is null(is nothing there)      

      this.getUserInput(); //get input from user
    } else {                                                 //else is something in local Storage  
      this.convertJson(this.localLocation);                  //call Server
